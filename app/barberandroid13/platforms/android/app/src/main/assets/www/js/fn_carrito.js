...
var idusuario=localStorage.getItem('id_user');
var datos="idusuario="+idusuario;
var pagina="ActualizarValoresCarrito.php";
...
var html=` <div class="sheet-modal my-sheet-swipe-to-close1" style="height:70%;background: black;">
    ...
    color: #c7aa6a;"></span>
      ...
      html+=`
         <div class="tarjeta contadortarjeta cambiarfuente  " style="width:100%;"  onclick="ElegirCortesia(`+idcarrito+`,0,'Ninguna')">
...
/*
    ...
    for (var i = 0; i < respuesta.length; i++) {
      ...
      html+=`
        <div class="tarjeta contadortarjeta cambiarfuente  " style="width:100%;"   onclick="ElegirCortesia(`+idcarrito+`,`+respuesta[i].idcortesia+`,'`+respuesta[i].nombrepaquete+`')">
              <div class="card demo-card-header-pic" style="border-radius: 10px;">  
...
function ElegirCortesia(idcarrito,idcortesia,nombrecortesia) {
    idcortesiaelegida=0;
    ...
    color: #c7aa6a;"></span>
                 ...
                 <div class="" style="position: absolute;top:1em;width: 100%;">
                        ...
                        `;
                          ...
                          <div class="row margin-bottom " style="padding-top: 1em;    margin-left: 2em;margin-right: 2em;margin-top:20px;">
                            <div class="col-50">
                            <button style="background: #C7AA6A;color:white;" type="button" class="button button-fill color-theme button-large button-raised  cambiarfuente" onclick="GuardarCortesiaCarrito(`+idcarrito+`,`+idcortesiaelegida+`)">Si</button>
                            </div>
...
function ObtenerCuponesAutomaticos() {
  
  var idusuario=localStorage.getItem('id_user');
  var idsucursal=localStorage.getItem('idsucursal');
  var datos="idusuario="+idusuario+"&idsucursales="+idsucursal;
  var pagina="ObtenerCuponesDisponiblesAutomaticos.php";
 ...
 $.ajax({
    ...
    async:false,
    success: function(resp){
      var cupones=resp.cupones;
      if (cupones.length>0) {

        AbrirModalCupones2(cupones);
      }
...
function AbrirModalCupones3(respuesta) {
...
var html=` <div class="sheet-modal my-sheet-swipe-to-close1" style="height:70%;background: black;">
    ...
    for (var i = 0; i < respuesta.length; i++) {
      ...
      html+=`
        <div class="tarjeta contadortarjeta cambiarfuente  " style="width:100%;"   onclick="ElegirCupon(`+respuesta[i].idcupon+`,'`+respuesta[i].codigocupon+`')">
              <div class="card demo-card-header-pic" style="border-radius: 10px;background:none!important;">  

            <div class="cardcortesia" id="cardcortesia_`+respuesta[i].idcupon+`" style="display: inline-table;
    justify-content: center;
    align-items: center;text-align: center;height: 50px;border:1px solid #9c9c9c; font-size: 18px;width:100%;   
   
    border-radius: 10px 10px 10px 10px;
    ">
    <div style="width:100%;margin-bottom:10px;">
           <p style="margin:10px 0px 0px 0px;text-align:center;color: white;font-size: 22px;">`+respuesta[i].codigocupon+` </p> 
         `
         ...
         ;
            ...
            if (respuesta[i].tipodescuento==0) {
              //porcentaje
                html+=`  <p class="item-text">
                       <span style="font-weight:bold;">Obtén `+Math.trunc(respuesta[i].descuento)+`% de descuento</span>

                </p>`;
            }

            if (respuesta[i].tipodescuento==1) {
              //monto
              html+=`<p class="item-text">
                        <span style="font-weight:bold;"> Obtén $`+respuesta[i].descuento+` de descuento</span>

                </p>`;
            }


            html+=`<p class="item-text" style="margin:0;">Válido hasta el `+respuesta[i].fechafinalformato +`</p>`;
               

                if (respuesta[i].horainicial!='') {
              html+=`  <p class="item-text" style="margin:0;">
                  A partir de `+respuesta[i].horainicial+` a `+respuesta[i].horafinal+` Hrs.

                </p>`;
            }


             if (respuesta[i].paquetes.length>0) {

               html+=`  <p class="">
                        <span style="font-weight:bold;"> Válido en el/los productos</span>

                </p>`;


                for (var j = 0; j <respuesta[i].paquetes.length; j++) {
                  html+=`<p class="" style="margin:0;">
                    -`+respuesta[i].paquetes[j].nombrepaquete+`
    ...
    dynamicSheet20 = app.sheet.create({
        content: html,
...
function AbrirModalCupones2(respuesta) {
  console.log(respuesta);
...
var html=` <div class="sheet-modal my-sheet-swipe-to-close1" style="height:70%;background: black;">
 ...
 if (respuesta.length>0) {
    ...
    for (var i = 0; i < respuesta.length; i++) {
          ...
          var imagen="";
        /*if (respuesta[i].foto!='' && respuesta[i].foto!=null) {
             imagen=urlimagenes+`cupones/imagenescupon/`+codigoserv+respuesta[i].foto;
      ...
      html+=`
        <div class="tarjeta contadortarjeta cambiarfuente  " style="width:100%;"   onclick="ElegirCupon(`+respuesta[i].idcupon+`,'`+respuesta[i].codigocupon+`')">
              <div class="card demo-card-header-pic" style="border-radius: 10px;background:#c7aa6a!important;">  
              <div style="background-image:url(`+imagen+`);border-radius: 10px 10px 0px 0px;display:block!important;" class="card-header ">
            ...
            if (respuesta[i].tipodescuento==0) {
              //porcentaje
                html+=`  <p class="item-text" style="text-align:center;">
                       <span style="font-weight:bold;">Obtén `+Math.trunc(respuesta[i].descuento)+`% de descuento</span>
            ...
            html+=`<p class="item-text" style="margin:0;text-align:center;">Válido hasta el `+respuesta[i].fechafinalformato +`</p>`;
               

                if (respuesta[i].horainicial!='') {
              html+=`  <p class="item-text" style="margin:0;text-align:center;">
                  A partir de `+respuesta[i].horainicial+` a `+respuesta[i].horafinal+` Hrs.
            ...
            <div class="cardcortesia" id="cardcortesia_`+respuesta[i].idcupon+`" style="display: flex;
    justify-content: center;
    align-items: center;text-align: center;height: 50px;background:#9c9c9c ;font-size: 18px;   
    border-bottom-left-radius: 10px;
    border-bottom-right-radius: 10px;">
           <p style="margin:0px;text-align:center;color: white;">`+respuesta[i].codigocupon+` </p> </div>
          </div>
          </div>
...
function ElegirCupon(idcupon,codigocupon) {
  
   var cuponcliente=codigocupon;

   if (idcupon!=0 && codigocupon!='' || cuponcliente!='') {
   var cuponcliente1="";

      if (idcupon!=0 && codigocupon!='') {

        cuponcliente1=codigocupon;

      }else{

        cuponcliente1=cuponcliente;
      }


    //   app.dialog.confirm('','¿Está seguro  de aplicar el cupón '+cuponcliente1+'?' , function () {
   
   $("#cuponcliente").val(cuponcliente1);
    var idcliente=localStorage.getItem('id_user');
    var idsucursales=localStorage.getItem('idsucursal');
  
   $(".cuponseleccionado").text('');


    localStorage.setItem('codigocupon',cuponcliente1);

    var datos="idcupon="+idcupon+"&codigocupon="+cuponcliente1+"&iduser="+idcliente+"&idsucursal="+idsucursales;
