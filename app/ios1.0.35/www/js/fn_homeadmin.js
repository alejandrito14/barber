                        ...
                        if(respuesta.checkin==1 && (respuesta.finalizacita=='' || respuesta.finalizacita==null) ) {
                          ...
                          if (respuesta.estatus==0 && respuesta.checkin==0 && respuesta.cancelacion==0) {
                            ...
                            html+=`<div class="checkinmanual"></div>`;
                               html+=`<button class="button button-large button-raised cambiarfuente"  style="background:#C7AA6A;color:white;margin-bottom:1em;" onclick="RealizarCheckinAdmin(`+respuesta.idcita+`)">Check-in</button>`;
                          

                          }

                           if(respuesta.checkin==0  && respuesta.checkout==0 && respuesta.cancelacion==0 && respuesta.estatus==0) {
                             html+=`
                                <div class="col-100">
                                  <button style="background: rgb(199, 170, 106); display: block;" type="button" class="button button-fill color-theme button-large button-raised btncontinuarcita cambiarfuente" 
	 ...
	 var monthNames = ['Enero', 'Febrero', 'Marzo', 'Abrirl', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'];
      calendarInline2 = app.calendar.create({
        ...
        on: {
          ...
          $(".calendar-day-today").addClass('calendar-day-selected');
...
function RealizarCheckinAdmin(idcita) {
  var aviso='';
    ...
    color: #c7aa6a;"></span>
                 ...
                 <div class="" style="position: absolute;top:1em;width: 100%;">
                        ...
                        `;
                          ...
                          <div class="row margin-bottom " style="padding-top: 1em;    margin-left: 2em;margin-right: 2em;margin-top:20px;">
                            <div class="col-50">
                            <button style="background: #C7AA6A;color:white;" type="button" class="button button-fill color-theme button-large button-raised  cambiarfuente" onclick="AceptarCheckinAdmin(`+idcita+`)">Si</button>
                            </div>
...
function AceptarCheckinAdmin(idcita) {
  dynamicSheet5.close();
  dynamicSheet10.close();

  var iduser=localStorage.getItem('id_user');
  var datos="idcita="+idcita+"&iduser="+iduser;
  var pagina = "AceptarCheckinAdmin.php";
  $.ajax({
    type: 'POST',
    dataType: 'json',
    url: urlphp+pagina,
