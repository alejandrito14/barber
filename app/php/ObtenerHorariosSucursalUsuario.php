...
try
{
	...
	$obtenersucursal=$especialista->AccesoSucursalEspecialistaHorario();
	
	$idsucursal=$obtenersucursal[0]->idsucursal;
	$idespecialista=$obtenersucursal[0]->idespecialista;
	...
	//var_dump($obtenerintervalos);die();
	/*for ($i=0; $i <count($obtenerzonas) ; $i++) { */
		$obtenerzonas[0]->idespecialista=$idespecialista;

		$obtenerzonas[0]->intervalos=array();
...
$horaactual=date('H:i');
		...
		for ($k=0; $k <count($obtenerintervalos); $k++) {
				...
				if ($hora1intervalo!='' && $hora2intervalo!='' && $pasa==1 ) {
					...
					array_push($obtenerzonas[0]->intervalos, $arrayintervalo);
