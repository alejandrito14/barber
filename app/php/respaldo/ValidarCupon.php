			 ...
			 $variabledescuento="";
				...
				if ($agregar==1) {
						...
						for ($i=0; $i < count($carrito); $i++) { 	
							...
							$carrito[$i]->{'totpaq'}=$carrito[$i]->{'price'}-$division;
							
								}else{

									$carrito[$i]->{'totpaq'}=$carrito[$i]->{'price'};
								}
						...
						$montopaquetes=$montoadescontar;

						$total=$totalcarrito-abs($montopaquetes);
		

		$nuevostotales=array('montopaquetes'=>$montoadescontar,'nuevototal'=>$total,'ventas'=>($comprascliente-1),'totalcarrito'=>$totalcarrito,'tipodescuento'=>$cupones[0]->tipodescuento,'variabledescuento'=>$variabledescuento);
