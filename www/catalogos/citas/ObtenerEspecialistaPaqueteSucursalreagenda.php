...
try
{
	...
	$citas->idcita=$idcita;
	$obtenerdetallecita=$citas->ObtenerdetallecitaAdmin();
	$idespecialista=$obtenerdetallecita[0]->idespecialista;
	...
	$obtenerespecialistas=$especialista->ObtenerEspecialistas2($idespecialistasele);
...
*/
		...
		if ($encontrado==0) {
			...
			$verificar=$citas->VerificarFechaHorarioEspecialistaCita();
			...
			array_push($especialistasdisponibles,$objeto);
			}
		}
	...
	for ($i=0; $i < count($especialistasdisponibles); $i++) { 
		...
		//print_r($especialistasdisponibles[$i]);
		if($fotoperfil==""){
