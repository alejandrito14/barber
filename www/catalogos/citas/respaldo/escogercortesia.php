...
<div class="row" style="margin-top: 1em;">
    ...
    display: flex;
    flex-wrap: nowrap;">
        <button type="button" class="btn btn-primary prev-step" style="display: none;" onclick="goToPreviousStep()">Regresar</button>

         <button type="button" class="btn btn-success btnstep6"  onclick="GuardarCortesia()" id="btnstep6" onclick="">Guardar</button>
...
<div class="row" style="margin-top: 1em;">
    ...
    flex-wrap: nowrap;">
        <button type="button" class="btn btn-primary prev-step" onclick="goToPreviousStep()" style="display: none;">Regresar</button>
         ...
         <button type="button" class="btn btn-primary btnstep7" id="btstep7">Guardar</button>
...
<script>
  ...
  var idcategoriapaquete='<?php echo $idcategoriapaquete; ?>';
   ...
   var detalle=[];
   var currentStep = 1;
   var idpaqueteseleccionado='<?php echo $idpaquete;?>';
  ...
  if (respuesta.length>0) {
   var foto="";
    ...
    for (var i = 0; i <respuesta.length; i++) {
      ...
      html+=`
        <div class="tarjeta cambiarfuente faustina mx-2" id="tarjetac_`+respuesta[i].idpaquete+`" onclick="SeleccionarCortesia(`+respuesta[i].idpaquete+`,`+respuesta[i].idcortesia+`,'`+respuesta[i].nombrepaquete+`')" style="width: 30%;">
          <div class="card demo-card-header-pic" style="border-radius: 10px;">
           <div class="card-header align-items-flex-end" style="background-image: url(`+foto+`); border-radius: 10px 10px 0px 0px; width: auto; height: 100px; background-repeat: round;">
            </div>
...
function SeleccionarCortesia(idpaquete,idcortesia,nombrecortesia) {

  $(".divcortesias").removeClass('activo');
  $("#divcortesias_"+idpaquete).addClass('activo');
...
function GuardarCortesia() {
  var idcita='<?php echo $idcita; ?>';
    var datos="idcita="+idcita+"&idcortesia="+idcortesiaseleccionado;
   $.ajax({
    type: 'POST',
    dataType: 'json',
    url:'catalogos/pagos/CambiarCortesia.php', //Url a donde la enviaremos
    data: datos, 
    success: function(datos){
       $("#modal-forms2").modal('hide');

       var html="Se realizó el cambio correctamente";
