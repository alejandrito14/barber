...
try
{
	...
	for ($i=0; $i < count($obtener); $i++) { 
		...
		$horainicial=strtotime($obtener[$i]->horainicial);
		$horafinal=strtotime($obtener[$i]->horafinal);

		$diferencia_en_segundos = $horafinal - $horainicial;
						$diferencia_en_minutos = $diferencia_en_segundos / 60;
		$obtener[$i]->diferencia=	$diferencia_en_minutos;	
	}
