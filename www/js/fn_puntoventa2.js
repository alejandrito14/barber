function EnviarPuntoVenta2(horainicial,fecha,idespecialista) {
		...
		var regresar='catalogos/puntoventa/vi_puntoventa.php?'+datos;
	    var donde='main';
...
function EnviarPuntoVenta3(horainicial,fecha,idespecialista,venderproductos) {
		...
		//clearInterval(totalcitasintervalo);
		//$(".btnseleccionarcliente").attr('onclick','CrearSesionUsuario('+idcliente+')');
		var datos='clienteseleccionado='+clienteseleccionado+'&horainicial='+horainicial+"&fecha="+fecha+"&idespecialistaselect="+idespecialista+"&venderproductos="+venderproductos;
		var regresar='catalogos/puntoventa/vi_puntoventa.php?'+datos;
	    var donde='main';
...
function Guardarpagos_puntovent(form,regresar,donde,idmenumodulo)
{ 
...
function SeleccionarClientePagos_puntoventa(idcliente) {
	//LimpiarVariables();
	clienteid=idcliente;
...
function ObtenerClientePagos_puntoventa(idcliente) {
...
function ObtenerDatosCliente_puntoventa(idcliente) {
...
function ObtenerMonedero_puntoventa(idcliente) {
	var datos="idusuario="+idcliente;
...
function EliminarCarritoCliente_puntoventa(idcliente) {
	var datos="idcliente="+idcliente;
	 $.ajax({
...
//punto venta
function PintarPagosPorpagar_puntoventa(pagos) {
	var html="";
	if (pagos.length>0) {
...
function Advertencia_puntoventa(idpago) {
...
function CambiarChek_puntoventa(argument) {
...
function PintarpagosTabla_puntoventa(respuesta) {
	var html="";
...
function SeleccionarTodosPagos_puntoventa() {
...
function VerificarDescuento_puntoventa() {
	 pagos=[];
	 //console.log(pagos);
	 var suma=0;
	$(".pagosusuario").each(function( index ) {
...
function PintarDescuentos_puntoventa(respuesta) {
	var html="";
	if (respuesta.length>0) {
...
function ObtenerMembresiaUsuario_puntoventa() {
	var idusuario=0;
...
function PintarMembresiasDescuento_puntoventa(respuesta) {
	var html="";
	if (respuesta.length>0) {
		for (var i = 0; i <respuesta.length; i++) {
...
function CalcularTotales_puntoventa() {
	var suma=0;
	pagos=[];
...
function ElegirMetodoPago_puntoventa() {

	//$("#modalmetodopago").modal();
	ObtenerTipodepagos();
}

function ObtenerTipodepagos_puntoventa() {
			$.ajax({
...
function ObtenerTipodepagosCompletar_puntoventa(idtipodepago) {
	$.ajax({
...
function PintarTipoPagos2_puntoventa(respuesta,idtipodepago) {
	var html="";
	/*if (respuesta.length>0) {
...
function PintarTipoPagos3_puntoventa(respuesta,idtipodepago) {
	var html="";
	/*if (respuesta.length>0) {
		html+=`<option value="0">SELECCIONAR TIPO DE PAGO</option>`;
		for (var i = 0; i <respuesta.length; i++) {
...
function CargartipopagoFactura_puntoventa(tipodepagoseleccionado) {
...
function PintarTipoPagos_puntoventa(respuesta) {
	var html="";
	/*if (respuesta.length>0) {
		html+=`<option value="0">SELECCIONAR TIPO DE PAGO</option>`;
		for (var i = 0; i <respuesta.length; i++) {
...
function SeleccionarTipodePago_puntoventa(idtipodepago) {
	CargarOpcionesTipopago(idtipodepago);
}

function SeleccionarTipodePago2_puntoventa(idtipodepago) {
	CargarOpcionesTipopago2(idtipodepago);
}

function SeleccionarTipodePago3_puntoventa(idtipodepago) {
	CargarOpcionesTipopago3(idtipodepago);
}


function ValidacionMonto_puntoventa() {
...
function HabilitarBotonPagar_puntoventa() {
   var seleccion=0;
...
function AbrirModalDetalle_puntoventa(idnotapago,idusuario) {
	$("#modaldetallenota").modal();
	ObtenerDetalleNota(idnotapago,idusuario);
}

function ObtenerDetalleNota_puntoventa(idnotapago,idusuario) {
...
function PintarDescuentosDetalle_puntoventa(respuesta) {
	var html="";
...
function PintarDescuentosDetalleMembresia_puntoventa(respuesta) {
	var html="";
	if (respuesta.length>0) {
		for (var i = 0; i <respuesta.length; i++) {
...
function PintardetalleNota_puntoventa(respuesta,usuario) {
...
function HabilitarOpcion_puntoventa(opcion) {
...
function ObtenerServiciosListado_puntoventa() {
      $.ajax({
      type: 'POST',
      dataType: 'json',
...
function PintarListadoServicio_puntoventa(respuesta) {
	if (respuesta.length>0) {
		var html="";
		for (var i = 0; i <respuesta.length; i++) {
...
function ObtenerMembresiaListado_puntoventa() {
      $.ajax({
      type: 'POST',
      dataType: 'json',
...
function PintarListadoMembresia_puntoventa(respuesta) {
	if (respuesta.length>0) {
		var html="";
		for (var i = 0; i <respuesta.length; i++) {
...
var idmenumodu=0;
function VerdatelleNota_puntoventa(idnotapago,idusuario,idmenumodulo) {
...
function CambiarEstatusNota_puntoventa(idnotapago,idusuario) {
estatus=['PENDIENTE','ACEPTADO','CANCELADO'];
...
function AceptarCambiarEstatus_puntoventa(idnotapago,idusuario) {
	...
	if ($("#btnState_1").is(':checked')) {
      ...
      success: function(resp){
      	...
      	VerdatelleNota_puntoventa(idnotapago,idusuario,idmenumodu);
...
function CerrarModalConfirm_puntoventa(idnotapago,estatus) {
		$("#modalconfirmacion").modal('hide');
		if (estatus==1) {
			estatus=0;
		}else{
...
function ObtenerListadoDatosFiscales_puntoventa() {
	if (idparticipante>0) {
  if ($("#requierefactura").is(':checked')) {
      ...
      success: function(resp){
       var respuesta=resp.respuesta;
       PintarListadoDatosFiscales_puntoventa(respuesta);
...
function PintarListadoDatosFiscales_puntoventa(respuesta) {
  	var html=`
...
function SeleccionarChekboxDatos_puntoventa(idusudatosfiscal) {
...
function AbrirModalDatoFiscal_puntoventa() {
...
function ObtenermetodoPago_puntoventa() {
...
function ObtenerformaPago_puntoventa() {
...
function ObtenerUsoCfdi_puntoventa() {
...
function GuardarDatoFiscal_puntoventa() {
  ...
  $.ajax({
    ...
    $("#modaldatofiscal").modal('hide');
      ObtenerListadoDatosFiscales_puntoventa();
...
function ObtenerClientesFiltro_puntoventa() {
	 ...
	 dataType:'json',
	  ...
	  success:function(msj){
		 ...
		 $("#buscadoralumnos_").val('');
		  PintarUsuariosAlumnosPunto_puntoventa(resp);
...
function PintarUsuariosAlumnosPunto_puntoventa(respuesta) {
	var html="";
	if (respuesta.length>0) {
...
function SeleccionarCliente_puntoventa(idcliente,nombre) {
	// body...
	 if($("#inputcli_"+idcliente+"_").is(':checked')){
...
function CrearSesionUsuario_puntoventa(idcliente) {
	...
	$.ajax({
	 ...
	 async:false,
	  ...
	  success:function(msj){
	  	...
	  	SeleccionarClientePagos_puntoventa(idcliente);
		$(".eleccion").css('display','block');	
			}
	});
}

function AbrirModalDetalleOperacion_puntoventa() {
	// body...
	...
	//if (NtabName=='punto-venta') {
		...
		CargarProductosSeleccionados2_puntoventa('tbllistardetalle');
	//}

	//if (NtabName=='pagos') {

		CargarPagosElegidosSeleccionados2_puntoventa('tbllistardetalle');
...
function CargarProductosSeleccionados2_puntoventa(tbllistarseleccionado) {
...
function ObtenerSucursales_puntoventa(idsucursalseleccionada) {
	 ...
	 dataType:'json',
	  ...
	  success:function(msj){
	  	var respuesta=msj.respuesta;
	  			...
	  			PintarSucursalesP_puntoventa(respuesta,idsucursalseleccionada);
	  			CrearSesionSucursal_puntoventa(idsucursalseleccionada)

			}
	});
}

function PintarSucursalesP_puntoventa(resp,idsucursalseleccionada) {
	var html="";
	if (resp.length>0) {
...
function CrearSesionSucursal_puntoventa(idsucursal) {
...
function AgregarNuevo_puntoventa(valor) {
...
function AgregarNuevo2_puntoventa(horainicial,fechaconsulta,
idespecialista) {
...
function ValidarCelularInput_puntoventa() {
...
function ValidarCelularInput2_puntoventa() {
	 ValidarCelular().then(r => {
		  		$("#validacioncelular").text('');
...
function AbrirModalAgendar_puntoventa(idcategoriapaquete) {
	 ...
	 $('#modal-forms2').on('shown.bs.modal', function () { 
			...
			ConsultarFechasCalendarioA_puntoventa();
		
				Obtenerdatostitulo(fechaselecte,horainicialselect,idespecialistaselect);
	...
	$.ajax({
		...
		success: function(resp){
			...
			$("#titulo-modal-forms2").css('width','100%');
...
function Vender_puntoventa(argument) {
	$("#buscador").css('display','block');
}

function ConsultarFechasCalendarioA_puntoventa() {
 ...
 //PintarFechaActual();
		 ...
		 var fecha=new Date();
		        ...
		        ObtenerFechasCalendarioAgendaA_puntoventa(anio,mes);
		        $("#txttitle").css('display','none');
...
function ObtenerFechasCalendarioAgendaA_puntoventa(anio,mes) {
...
function PintarFechaSeleccionadaA_puntoventa(fecha) {
...
function ObtenerCategoriasPrincipales_puntoventa(venderproductos) {
	
	var data='venderproductos='+venderproductos;
	$.ajax({
	 ...
	 data:data,
	  ...
	  success:function(msj){
	  			...
	  			PintarCategorias_puntoventa(respuesta);

			}
	});
}

function PintarCategorias_puntoventa(respuesta) {
	var html="";
	if (respuesta.length>0) {
		for (var i = 0; i < respuesta.length; i++) {
				//funcion="SeleccionarServicio("+respuesta[i].idcategoriapaquete+")";
				funcion="AbrirModalAgendar_puntoventa("+respuesta[i].idcategoriapaquete+")";
...
function ModificarPrecio_puntoventa(idcarrito) {
	
	$("#modalprecio").modal();
	$("#txtprecio").val('');
	$("#btnmodificar").attr('onclick','ModificarPrecioCarrito_puntoventa('+idcarrito+')')
}

function ModificarPrecioCarrito_puntoventa(idcarrito) {
	var valor=$("#txtprecio").val();
	var datos="idcarrito="+idcarrito+"&precio="+valor;


	AbrirModalValidacion_puntoventa(idcarrito,valor);
	phoneFormatter2('txtusuario');

}

function AbrirModalValidacion_puntoventa(idcarrito,valor) {
...
function GuardarModificar_puntoventa(idcarrito,valor) {
	  ...
	  success:function(msj){
				...
				$("#txtusuario").val('');
	  			if (msj.respuesta==1) {
	  				...
	  				ObtenerPaquetesCarrito_puntoventa();
	  			}else{
...
function Obtenerdatostitulo(fecha,hora,idespecialista) {
	var datos="fecha="+fecha+"&hora="+hora+"&idespecialista="+idespecialista;

		$.ajax({
	 url:'catalogos/puntoventa/Obtenesdatosseleccionados.php', //Url a donde la enviaremos
	 type:'POST', //Metodo que usaremos
	 dataType:'json',
	 data:datos,
	  error:function(XMLHttpRequest, textStatus, errorThrown){
	  ...
	  success:function(msj){
	  	var especialista=msj.especialista;
	  	var nombreusuario=msj.usuario;
	  	var fecha=msj.fecha;
	  	var hora=msj.hora;
	  		var html=`Servicios
	  		<div class="row" style="margin-left: 2px;text-align:center;display: flex;
    justify-content: center;">Cliente: `+nombreusuario+`</div>
	  		<div class="row" style="margin-left: 2px;text-align:center;display: flex;
    justify-content: center;">
	  			Fecha: `+fecha+`|Hora: `+hora+`|`

	  		+especialista[0].nombre+` `+especialista[0].paterno+`

	  			</div>`;
	  		$("#titulo-modal-forms2").html(html);
