...
function AbrirModalTarjetaUsuario(idtarjetalealtad) {
 	...
 	//ObtenerUsuariosAsignados(idtarjetalealtad);
 	    aparecermodulos('catalogos/tarjetalealtad/detalletarjeta.php?idtarjetalealtad='+idtarjetalealtad,'main');
 ...
 function PintarTarjetaAsignadas(respuesta) {
 	...
 	if (respuesta.length>0) {
 		for (var i = 0; i <respuesta.length; i++) {
 			var citas=[];
 			if (respuesta[i].citastarjeta!='' && respuesta[i].citastarjeta!=null) {
 			 citas=respuesta[i].citastarjeta.split(',');
 			}
 			html+=`
 				<tr>
	 				<td>
				  ...
				  var cantidadproducto=respuesta[i].cantidadproducto;
				  	...
				  	if(parseInt(cantidadproducto)>=parseInt(cantidadrequerida) && respuesta[i].estatus==0) {
				  		respuesta[i].estatus=3;
				  	}else{

				  		if(parseInt(cantidadrequerida)==parseInt(cantidadproducto) && respuesta[i].estatus==0) {
				  		respuesta[i].estatus=3;
				  		}
				  	}


				  /*	if (respuesta[i].idusuarios==824) {
				  		alert(cantidadproducto+''+cantidadrequerida);
				  		alert(respuesta[i].estatus);
				  	}*/
...
*/				   
 ...
 function AbrirModalTarjeta(idtarjetalealtad) {
 	...
 	//ObtenerUsuariosAsignadosVisita(idtarjetalealtad);
 	 	    aparecermodulos('catalogos/tarjetalealtad/detalletarjetaVisita.php?idtarjetalealtad='+idtarjetalealtad,'main');
	...
	if (!$.fn.DataTable.isDataTable('#tbltarjetausuario2')) {
	    // Si no tiene DataTables aplicado, inicializar DataTables
	 	...
	 	cargarTabla2();
           ...
           var table = $('#tbltarjetausuario2').DataTable();
		   $('#tbltarjetausuario2_filter input').val('');
                // Reset the search filter in DataTables
           table.search('').draw();
	}
