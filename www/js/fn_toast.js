...
function elementotoast(tipo, titulo, descripcion, autoCierre,idcita ) {
	...
	const numeroAlAzar = Math.floor(Math.random() * 100);
	const fecha = Date.now();
	const toastId = 'noticita_'+idcita;
	nuevoToast.id = 'noticita_'+idcita;
	...
	const iconos = {
		...
		cita:`<span style="font-size:30px;" class="mdi mdi-cellphone"></span>`,
	};
	...
	if (autoCierre) {
		...
		nuevoToast.click(function (argument) {
			alert('a');
...
function cerrarToast(id) {
 		document.getElementById(id)?.classList.add('cerrando');
 		var idcita=id.split('_')[1];
 		var datos="idcita="+idcita;

 		$.ajax({
					url: 'catalogos/dashboard/GuardarVistoCita.php', //Url a donde la enviaremos
